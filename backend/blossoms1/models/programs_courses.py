...
from backend.blossoms1.models.programs_model import *
...
class Programs_courses(db.Model):
    ...
    def add_program_course(_prog, _course_code, _sem):
        new_prog_course = Programs_courses(
            course_code=_course_code, program_name=_prog, sem=_sem)
        ...
        if len(course_exists) > 0 and len(program_exists) > 0:
            prog_course_already_added = Programs_courses.get_prog_course_rel(
                _course_code=_course_code, _prog=_prog, _sem=_sem)
    ...
    def get_courses_for_prog_for_sem(_prog, _sem):
        result = Programs_courses.query.filter_by(
            sem=_sem, program_name=_prog).all()
        print(result)
        if result == None:
            return []
    ...
    def get_prog_course_rel(_course_code, _prog, _sem):
        result = Programs_courses.query.filter_by(
            course_code=_course_code, program_name=_prog, sem=_sem).first()
        print(result)
        if result == None:
            return []
    ...
    def update_prog_course(_code, _sem, _prog, _new_code=None, _new_prog=None, _new_sem=None):
        prog_course_added = Programs_courses.get_prog_course_rel(
            _course_code=_code, _prog=_prog, _sem=_sem)
        if len(prog_course_added) > 0:
            prog_course_to_update = Programs_courses.query.filter_by(
                course_code=_code, program_name=_prog, sem=_sem).first()
            if _new_prog != None:
    ...
    def delete_prog_course(_course, _sem, _prog):
        prog_cousrse_added = Programs_courses.get_prog_course_rel(
            _course_code=_course, _prog=_prog, _sem=_sem)
        if len(prog_cousrse_added) > 0:
            Programs_courses.query.filter_by(
                course_code=_course, program_name=_prog, sem=_sem).delete()
            db.session.commit()
            print('deleted')
            return 'deleted'
        else:
...
if __name__ == '__main__':
    # db.create_all()
    print(Programs_courses.delete_prog_course(
        _course='Math 351', _prog='computer', _sem=5))
