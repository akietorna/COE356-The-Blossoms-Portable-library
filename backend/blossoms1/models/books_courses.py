...
from backend.blossoms1.models.books_model import *
...
class Books_courses(db.Model):
    ...
    # todo foreign key todo either book link or id
    book_link = db.Column(db.String(500), nullable=False)
    ...
    def add_book_course(_link, _course_code):
        new_book_course = Books_courses(
            book_link=_link, course_code=_course_code)
        ...
        if len(book_exists) > 0 and len(course_exists) > 0:
            relationship_already_added = Books_courses.get_books_for_course(
                _course_code=_course_code)
    ...
    def get_book_course_rel(_course_code, _link):
        results = Books_courses.query.filter_by(
            course_code=_course_code, book_link=_link).first()
        print(results)
        if results == None:
            return []
    ...
    def update_book_course(_link, _course_code, _new_link):
        relationship_already_added = Books_courses.get_book_course_rel(
            _course_code=_course_code, _link=_link)
        if len(relationship_already_added) > 0:
            book_exists = Books.get_book(_link=_new_link)
            if len(book_exists) > 0:
                book_course_to_update = Books_courses.query.filter_by(
                    book_link=_link, course_code=_course_code).first()
    ...
    def delete_book_course(_link, _course_code):
        relationship_already_added = Books_courses.get_book_course_rel(
            _course_code=_course_code, _link=_link)
        if len(relationship_already_added) > 0:
            Books_courses.query.filter_by(
                book_link=_link, course_code=_course_code).delete()
            db.session.commit()
            print('deleted')
            return('deleted')
        else:
